...
- Host-prefixed shorthand: `github.com/owner/repository[/directory][@ref]`, `gitlab.com/...`, `bitbucket.org/...`, or `git.sr.ht/~owner/repository[/directory][@ref]`
...
The optional `ref` can be a tag, branch, or full 40-character commit SHA. If you omit it, Packref
uses the default branch and pins its current commit. Direct repository references are manual
references.

A project can select one directory for each repository and pinned commit identity. Remove the
existing package source reference before selecting another directory for that identity. Different
projects can select different directories while they reuse the same full-repository source
snapshot.
